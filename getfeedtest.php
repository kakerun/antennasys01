...
foreach ($feeds as $url){
	...
	echo($url['sitename']." ".$url['sitehash']);
	//$item = $url[0];
    $feed = new SimplePie();
    ...
    for ($x = 0; $x < $feed->get_item_quantity($items_per_feed); $x++){
		...
		if($item < strtotime($gdp->today) && $item >= strtotime($gdp->getday)){
			...
			echo($num." ".$first_items[$num]['sitename']." ".$first_items[$num]['articlename']." ".$first_items[$num]." ".$first_items[$num]['rssall']);
			++$cnt;
		}
    }
    unset($feed);
