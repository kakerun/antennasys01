...
//カテゴリ
//$category = $gct->c_detail();
//$category_rss = $gct->c_rss();
...
foreach ($feeds as $url){
    ...
    for ($x = 0; $x < $feed->get_item_quantity($items_per_feed); $x++){
		...
		if($item < strtotime($gdp->today) && $item >= strtotime($gdp->getday)){
			...
			//アンテナ別フィルター
			$filter_word = "none";
			foreach($ant_all as $ant_item){
				if($ant_item['antennaid'] === $url['antennaid']){
					$filter_word = $ant_item['filter'];
					break;
				}
			}
			if($filter_word === "none"){
...
//DB書込
$gdb->set_FeedData($file_path,$gdfa->TABLE_ARTICLE,$first_items,$table_create_flag);
